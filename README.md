...
The primary owner's **Access** tab grants access by a verified account email with a separate 6-12 digit PIN. The account must share a team with the primary owner. Re-granting changes that member's PIN and invalidates their console unlocks; revocation denies subsequent requests immediately. Losing all shared team memberships also denies access. Never share the primary owner's PIN. Changing the primary owner clears delegated grants.

Board mode shows saved positions, sizes, links, text, images, and expandable checklists with an isolated pan/zoom camera, minimap, search, and detail inspector. It is a read-only snapshot: use Refresh for newer data. It never writes to board stores, geometry, or presence. Image previews load on demand for visible cards above 25% zoom and can be disabled. Canvas snapshots are capped at 2,000 objects including tasks and links; larger boards remain available in paginated List mode. Canvas export includes the whole loaded snapshot, including layout and private content.
...
Необязательная панель находится в **настройках профиля** и доступна главному владельцу и назначенным им администраторам панели. Владельцы и администраторы команд автоматически этих прав не получают. В панели есть журнал с фильтрами, обзор пользователей и проектов, просмотр личных и проектных досок, экспорт. Только главный владелец может выдавать и отзывать доступ к панели, отзывать приглашения и удалять до 5000 событий старше 90 дней.

Во вкладке **Доступ** главный владелец указывает подтверждённый email участника своей команды и отдельный PIN из 6-12 цифр. Повторная выдача заменяет PIN участника и блокирует его открытые сессии панели. Отзыв запрещает следующие запросы сразу; потеря всех общих команд с главным владельцем тоже запрещает доступ. Не передавай свой PIN. Смена главного владельца удаляет выданные права панели.

Режим доски показывает сохранённые позиции, размеры, связи, тексты, изображения и раскрываемые To-do. Есть независимая камера, масштаб, миникарта, поиск и подробности. Это снимок только для чтения: кнопка обновления загружает новые данные. Просмотр не меняет рабочее состояние досок, геометрию и присутствие. Превью изображений загружаются для видимых карточек при масштабе от 25%; их можно отключить. Холст ограничен 2000 объектами, включая задачи и связи; большие доски доступны в постраничном списке. Экспорт холста содержит весь загруженный снимок, включая позиции и личные данные.
