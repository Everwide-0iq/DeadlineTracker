...
- Desktop canvas from `1024px`: pan, zoom, draggable DOM cards, desktop-only text layers, minimap navigation, alignment snapping, card links, heat horizon, presence cursors, and sidebar filters.
- Mobile list below `1024px`: deadline sorting, filter chips, floating create button, and no drag-and-drop.
- Card CRUD: create, edit, mark done/reopen, transfer active ownership, bulk delete, resize from content, attach one compressed image/screenshot, and connect cards with visual arrows.
- Active cards clearly show who is working on them; completed cards store an exact server timestamp and display it in each viewer's local time zone. Legacy completion dates remain explicitly unknown.
- Desktop context menu for creating a card or text layer exactly where you right-click the empty canvas.
...
6. Make sure Realtime is enabled for `public.cards`, `public.projects`, `public.card_links`, `public.board_texts`, and `public.profiles`. The migration also creates private `card-images` and `avatars` Storage buckets and attempts to add these tables to `supabase_realtime`.
...
- Desktop-доска от `1024px`: pan, zoom, draggable DOM-карточки, desktop-only текстовые слои, навигация по миникарте, snapping, стрелочные связи между карточками, heat horizon, курсоры участников и фильтры в sidebar.
- Mobile-список ниже `1024px`: сортировка по дедлайну, filter chips, floating-кнопка создания и без drag-and-drop.
- CRUD карточек: создать, редактировать, отметить готово/вернуть в работу, передать статус «Активно», массово удалить, автоматически расширять под контент, прикреплять одно сжатое изображение/скриншот и связывать карточки визуальными стрелками.
- Активные карточки выразительно показывают, кто над ними работает; завершённые карточки получают точное серверное время и показывают его в локальном часовом поясе зрителя. Для старых карточек дата честно помечается неизвестной.
- Контекстное меню desktop-доски для создания карточки или текста точно в месте клика по пустому canvas.
...
6. Проверь, что Realtime включён для `public.cards`, `public.projects`, `public.card_links`, `public.board_texts` и `public.profiles`. Миграция также создаёт приватные Storage buckets `card-images` и `avatars` и сама пытается добавить эти таблицы в `supabase_realtime`.
