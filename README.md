...
6. Make sure Realtime is enabled for `public.cards`, `public.projects`, `public.card_links`, and `public.board_texts`. The migration also attempts to add these tables to `supabase_realtime`.
...
6. Проверь, что Realtime включён для `public.cards`, `public.projects`, `public.card_links` и `public.board_texts`. Миграция также сама пытается добавить эти таблицы в `supabase_realtime`.
