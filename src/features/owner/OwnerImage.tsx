...
export function OwnerImage({ path, todo, t, inline = false }: { path: string; todo: boolean; t: OwnerCopy; inline?: boolean }) {
  ...
  return <div className={inline ? 'owner-image owner-image-inline' : 'owner-image'}>
    {!inline && <button className="secondary-button" type="button" onClick={() => setOpen(!open)}><Image size={15} />{open ? t.hideImage : t.image}</button>}
