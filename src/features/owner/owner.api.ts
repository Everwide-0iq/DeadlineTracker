...
export type OwnerStatus = { isOwner: boolean; canManageAccess: boolean; unlockedUntil: string | null; blockedUntil: string | null; ok?: boolean }
export type OwnerBoardTarget = { userId?: string; projectId?: string; name: string }
...
export function parseOwnerStatus(value: Json): OwnerStatus {
  ...
  return { isOwner: row.isOwner, canManageAccess: row.canManageAccess === true, unlockedUntil: timestamp('unlockedUntil'), blockedUntil: timestamp('blockedUntil'), ok: row.ok === true }
}
...
export async function ownerStatus(signal = AbortSignal.timeout(15_000)): Promise<OwnerStatus> {
  ...
  if (error?.code === 'PGRST202' || error?.code === '42883') return { isOwner: false, canManageAccess: false, unlockedUntil: null, blockedUntil: null }
...
export async function ownerAccess(action: 'list' | 'grant' | 'revoke', email?: string, pin?: string) {
  const { data, error } = await requireSupabase().rpc('owner_console_access', { action_name: action, target_email: email, new_pin: pin }).abortSignal(AbortSignal.timeout(15_000))
  if (error) throw error
  if (!Array.isArray(data)) throw new Error('Invalid access response')
  return data.map(asRow)
}
